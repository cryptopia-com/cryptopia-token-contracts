...
async function main() {
    ...
    {
        const [lzEndpointAltTokenContract] = await ensureDeployed(
            "MockLayerZeroEndpointAltToken", 
            [],
            "MockLayerZeroEndpointAltToken:Skale");
        const lzEndpointAltTokenAddress = await lzEndpointAltTokenContract.address;

        const [lzEndpointAltContract] = await ensureDeployed(
            "MockLayerZeroEndpointAlt", 
            [
                config.layerZero.endpoint.endpointId,
                lzEndpointAltTokenAddress
            ],
            "MockLayerZeroEndpointAlt:Skale");

        lzEndpointAddress = await lzEndpointAltContract.address;
    }
    else 
    {
